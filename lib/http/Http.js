...
var Http = {
    ...
    doExecute: function(config) {
        ...
        if (config.json==1 && config.method === METHOD_POST) {
        ...
        return axios(config).then(function(result) {
            ...
            if (response !== null && response !== '' && response !== undefined && response !== 'undefined') {
                // 对应code操作
                global.httpCode(response,config)
            }
    ...
    combine: function(config) {
        ...
        if (config.method === "POST") {
         config.data = config.form==1 ? config.data : ((config.json==1 ? JSON.stringify(config.data) : qs.stringify(config.data)) || {});
        }
        var isFormData = (config.method=="POST")
        if (!isFormData) {
            // 过滤掉null数据
